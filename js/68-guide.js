...
regPage('guide', function (root) {
  ...
  arch.appendChild(h('pre', { class: 'guide-pre' },
    ...
    '\n' +
    '   區管理系統＝只管連結＋紀錄（佢自己分頁貼 URL，自動掛載成員系統）\n' +
    '   成員系統＝填報名資料（寫入訓練班「表格回應」）\n' +
    '\n' +
    '   批核：區管理層連結 GS 睇資料 → tick「區會批准」格 → CL 見 ✔ 先出通告'));
  arch.appendChild(h('div', { class: 'row-sub' }, '呢個 APP 係純前端：資料全部喺每班嘅 Google Sheet（GS）度，APP 直接同 GS 對話，冇 server、冇戶口登記。'));
  ...
  const steps = [
    ...
    ['1️⃣1️⃣ 完成模組', '「🎓 完成」：出席率自動計 → 評核（✔/✗＋證書編號，可自動編號）→ 合格名單 → 列印證書 → 完成報告一掣寫入 GS → 領取證書紀錄。'],
  ];
  ...
  [
    ...
    ['資料喺邊？', '全部喺每班自己嘅 Google Sheet。APP 冇 server、唔會備份你啲資料去第二度；GS 就係唯一事實來源，區管理層隨時連結核對。'],
  ...
  admin.appendChild(h('div', { class: 'guide-step' },
    h('div', { class: 'guide-step-body' },
      ...
      h('br'),
      '⑤ 收款核對：setPaymentCheck API（tick「已核對收款」）；完成後讀「Print_訓練班完成報告」連結成員系統紀錄')));
