...
| 🆕 新開班＋掛載流程 | CL 填基本資料**即刻起真 GS**（區級 CourseFactory copy 模版；回傳 GS＋SCRIPT 網址俾 CL 複製交區管理系統貼佢自己分頁；首次密碼 1234）→ CL 喺 APP 填晒預算／節次／時間表／通告（直接寫入 GS）→ 區管理層批改後 tick「**區會批准**」格（訓練班 GS 參數分頁，APP 只讀）→ CL 見 ✔ 生成通告交區網頁管理員 → 上網後區管理系統自動掛載，報名流入（=已掛載信號）；儀表板「🚢 掛載流程」四步實時顯示 |
...
| ✅ 收生確認 | 待批／已取錄／拒絕／取消／💰未核對收款 篩選＋搜尋；詳情彈窗（家長・領袖・付款・入數紙 Drive 連結）；接納／拒絕／取消／還原待批（記批核人＋時間）；批量接納；超名額＋**未核對收款**防呆警告；💰✔/💰？ 收款核對狀態 |
...
| 💵 收支 | 支出逐筆入帳（Input04 收據 1-35，append-only 唔撞 rev）；班費收入自動計（×取錄人數／已核對收款實收）；盈虧即時；收據快滿提醒；列印收支計算表 |
...
| 📖 教學 | 三方分工架構圖＋CL 全流程逐步指引（每步做咩/去邊頁/撳咩掣）＋要向管理層攞嘅嘢（開班碼/通告檔案編號/訓練班電郵）＋FAQ（密碼/多人同時/衝突）＋🧪 一掣開演示班跟住做；另有區管理層版重點（getCourseSummary 批核） |
