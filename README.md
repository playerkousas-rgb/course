...
| 🆕 新開班＋掛載流程 | CL 開**本機草稿班**填晒預算／節次／時間表／通告 → 儀表板「📤 生成 GS 交區」（經區級 CourseFactory copy 模版＋寫入晒資料）→ 複製 URL 交區 → 區管理系統批改＋tick「區會批准」→ APP 生成通告交區網頁管理員 → 上網後區管理層貼「通告網址」→ 正式掛載成員系統報名；全程狀態喺儀表板「🚢 掛載流程」五步顯示（批准／通告網址格只讀，15 秒自動更新） |
