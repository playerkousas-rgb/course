...
>
> 本 repo 前端本身無中央 server；**每班 Google Sheet 旁邊嘅 Apps Script Web App 就係該班後端**。資料直接同該班 GS／Script 對話。
...
| 🆕 新開班＋掛載流程 | CL 填基本資料**即刻起真 GS**（區級 CourseFactory copy 模版；回傳 GS＋SCRIPT 網址俾 CL 複製交區管理系統貼佢自己分頁；首次密碼 1234）→ CL 喺 APP 填晒預算／節次／時間表／通告（直接寫入 GS）→ 區管理層批改後 tick「**區會批准**」格（訓練班 GS 參數分頁，APP 只讀）→ CL 見 ✔ 生成通告（通告印成員系統 direct 報名連結，不是 Script /exec）交區網頁管理員 → 上網後區管理系統自動掛載，報名流入（=已掛載信號）；儀表板「🚢 掛載流程」四步實時顯示 |
...
人手輸入範圍保持最少：訓練班系統只需要 Script URL / API Key；區管理系統只需要網頁通告 URL、Script URL、通告編號，其他就是批核。真正用來對準成員系統報名表的是 `publicCourseId`，由 CourseFactory 自動產生，區管理系統 CourseLinks 自動保存並掛載；成員系統不用人手設定。
