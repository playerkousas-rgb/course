...
| 🆕 新開班＋掛載流程 | CL 填基本資料**即刻起真 GS**（區級 CourseFactory copy 模版，回傳 GS URL 交區管理系統 SCRIPT 連結批核；首次密碼 1234）→ CL 喺 APP 填晒預算／節次／時間表／通告（直接寫入 GS）→ 區管理層批改＋tick「區會批准」→ APP 生成通告交區網頁管理員 → 上網後區管理層貼「通告網址」→ 正式掛載成員系統報名；儀表板「🚢 掛載流程」四步實時顯示（批准／通告網址格只讀，15 秒自動更新） |
